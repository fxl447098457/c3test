...
// ============================================================
// P6.13: IDL生成 (TypeLib嵌入)
// 从coclass信息生成IDL文件, 供MIDL编译成.tlb, 嵌入DLL资源
// ============================================================

std::string CCodeGen::generateIdl(const std::string& progId,
                                   const std::string& libidStr,
                                   const std::vector<SymbolTable*>& allSymTabs) {
    CodeEmitter idl;

    // 收集所有instancing >= PublicNotCreatable的类模块 (与generateDllEntry一致)
    struct CoClassInfo {
        std::string moduleName;
        std::string clsidStr;
        std::string progId;
    ...
    for (auto& [key, sym] : symTab_.moduleScope()->symbols()) {
        if (sym->kind == SymbolKind::Class && !sym->isInterface) {
            if (sym->instancing != VBInstancing::Private) {
                CoClassInfo info;
                info.moduleName = sym->name;
                info.clsidStr = sym->comClsidStr.empty() ? generateClsid(progId + "." + sym->name) : sym->comClsidStr;
                info.progId = progId + "." + sym->name;
    ...
    if (coClasses.empty()) {
        return "// P6.13: No public creatable classes found for IDL generation\n";
    }

    // VB6类型 → IDL类型映射
    auto mapIdlType = [](Vb6Type t) -> std::string {
        switch (t) {
            case Vb6Type::Long:    return "LONG";
            case Vb6Type::Integer: return "SHORT";
            case Vb6Type::Byte:    return "BYTE";
            case Vb6Type::Boolean: return "VARIANT_BOOL";
            case Vb6Type::Double:  return "DOUBLE";
            case Vb6Type::Single:  return "FLOAT";
            case Vb6Type::String:  return "BSTR";
    ...
    // 生成LibID (如果未指定则用FNV生成)
    std::string effectiveLibid = libidStr;
    if (effectiveLibid.empty()) {
        effectiveLibid = generateClsid(progId + ".TypeLib");
    }

    // === IDL文件 ===
    idl.emitLine("// Generated by vb6c3 (c3.exe) - TypeLib IDL");
    idl.emitBlank();

    // 1. library块
    idl.emitLine("[");
    idl.indent();
    idl.emitLine("uuid(" + effectiveLibid.substr(1, effectiveLibid.size() - 2) + "),");
    idl.emitLine("version(1.0),");
    idl.emitLine("helpstring(\"" + progId + " TypeLib\")");
    idl.dedent();
    idl.emitLine("]");
    idl.emitLine("library " + cIdent(progId));
    idl.emitLine("{");
    idl.indent();

    // importlib("stdole2.tlb") — 引入OLE Automation标准类型
    idl.emitLine("importlib(\"stdole2.tlb\");");
    idl.emitBlank();

    // DISPID分配 (与generateDllEntry一致)
    std::unordered_map<std::string, int> dispIdMap;
    int nextDispid = 1;

    // 2. 为每个coclass生成dispinterface + coclass
    for (auto& cc : coClasses) {
        std::string clsId = cIdent(cc.moduleName);

        // 收集该类的唯一属性名 (Get/Let共享同一属性名)
        // 收集唯一方法/属性用于dispinterface
        struct DispEntry {
            std::string bareName;  // 去掉模块前缀的方法名
            int dispid;
            Symbol* getSym;        // Property Get (可选)
            Symbol* letSym;        // Property Let (可选)
            Symbol* setSym;        // Property Set (可选)
            bool isMethod;         // Sub/Function
            Symbol* methodSym;     // 方法符号 (isMethod时有效)
        };
        std::vector<DispEntry> dispEntries;
        std::unordered_map<std::string, size_t> nameToEntry;
        ...
        for (size_t mi = 0; mi < cc.publicMethodNames.size(); mi++) {
            ...
            if (methodSym->kind == SymbolKind::Sub || methodSym->kind == SymbolKind::Function) {
                // 方法
                int dispid;
                auto dpIt = dispIdMap.find(lowerBareName);
                if (dpIt != dispIdMap.end()) {
                ...
                entry.dispid = dispid;
                entry.getSym = nullptr;
                entry.letSym = nullptr;
                entry.setSym = nullptr;
                entry.isMethod = true;
                entry.methodSym = methodSym;
                dispEntries.push_back(entry);
                nameToEntry[lowerBareName] = dispEntries.size() - 1;
            } else {
                // Property: 合并同名Get/Let/Set
                auto it = nameToEntry.find(lowerBareName);
                if (it != nameToEntry.end()) {
                    // 已有该属性名的条目
                    if (methodSym->kind == SymbolKind::PropertyGet) {
                        dispEntries[it->second].getSym = methodSym;
                    } else if (methodSym->kind == SymbolKind::PropertyLet) {
                        dispEntries[it->second].letSym = methodSym;
                    } else if (methodSym->kind == SymbolKind::PropertySet) {
                        dispEntries[it->second].setSym = methodSym;
                    }
                } else {
                    // 新建属性条目
                    int dispid;
                    ...
                    entry.letSym = nullptr;
                    entry.setSym = nullptr;
                    entry.isMethod = false;
                    entry.methodSym = nullptr;
                    if (methodSym->kind == SymbolKind::PropertyGet) entry.getSym = methodSym;
                    else if (methodSym->kind == SymbolKind::PropertyLet) entry.letSym = methodSym;
                    else if (methodSym->kind == SymbolKind::PropertySet) entry.setSym = methodSym;
                    dispEntries.push_back(entry);
                    nameToEntry[lowerBareName] = dispEntries.size() - 1;
                }
            }
        ...
        // dispinterface
        std::string ifaceName = "_" + clsId;
        idl.emitLine("dispinterface " + ifaceName + ";");
        idl.emitBlank();

        idl.emitLine("[");
        idl.indent();
        // 为dispinterface生成独立UUID
        std::string ifaceUuid = generateClsid(progId + "." + cc.moduleName + ".dispinterface");
        idl.emitLine("uuid(" + ifaceUuid.substr(1, ifaceUuid.size() - 2) + ")");
        idl.dedent();
        idl.emitLine("]");
        idl.emitLine("dispinterface " + ifaceName);
        idl.emitLine("{");
        idl.indent();
        idl.emitLine("properties:");
        idl.indent();

        // 属性 (Property Get/Let)
        for (auto& de : dispEntries) {
            if (de.isMethod) continue;
            if (!de.getSym && !de.letSym) continue;

            std::string propType = "VARIANT";
            if (de.getSym) {
                propType = mapIdlType(de.getSym->type);
            } else if (de.letSym && !de.letSym->params.empty()) {
                propType = mapIdlType(de.letSym->params.back().type);
            }

            idl.emitLine("[id(" + std::to_string(de.dispid) + ")] " + propType + " " + de.bareName + ";");
        }

        idl.dedent();
        idl.emitLine("methods:");
        idl.indent();
        ...
        for (auto& de : dispEntries) {
            ...
            std::string retType = de.methodSym->kind == SymbolKind::Function
                                  ? mapIdlType(de.methodSym->type) : "void";

            std::string params;
            for (size_t pi = 0; pi < de.methodSym->params.size(); pi++) {
                if (pi > 0) params += ", ";
                auto& p = de.methodSym->params[pi];
                if (!p.isByVal) params += "[in,out] ";
                else params += "[in] ";
                params += mapIdlType(p.type) + " " + p.name;
            }

            idl.emitLine("[id(" + std::to_string(de.dispid) + ")] " + retType + " " + de.bareName +
                        "(" + params + ");");
        }
        ...
        idl.indent();
        // 提取CLSID内容 (去掉花括号)
        std::string clsidContent = cc.clsidStr;
        if (clsidContent.size() >= 2 && clsidContent.front() == '{' && clsidContent.back() == '}') {
            clsidContent = clsidContent.substr(1, clsidContent.size() - 2);
        }
        idl.emitLine("uuid(" + clsidContent + ")");
        idl.dedent();
        idl.emitLine("]");
        idl.emitLine("coclass " + clsId);
        idl.emitLine("{");
        idl.indent();
        idl.emitLine("[default] dispinterface " + ifaceName + ";");
        idl.dedent();
        idl.emitLine("};");
        idl.emitBlank();
    }
