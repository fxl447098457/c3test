...
2. ~~x64 下是否放开「引用 VB 局部变量」（需自动 spill 到栈并映射）——当前限定仅参数。~~
   ...
   **不需要**新的栈槽映射 / 寄存器分配器；代价是 x64 单片段最多引用 4 个变量（3040）。
...
9. ~~混排 (项2) + 片段引用 VB 局部变量 (项3)~~ —— **已实现 (2026-09-24)**，见 §12.5。
   残留边界：类方法不支持（3037）；x64 单片段 ≤4 个变量引用（3040）；跨片段不共享寄存器/标志位；
   不支持 `me->` 成员引用。
10. ~~项1 cmpxchg×RAX/EAX 别名陷阱~~ —— **已工具化为 3041**（2026-09-24 晚），见 §11.1。
...
### 11.1 项1 已工具化（2026-09-24 晚）—— VB3041

上述陷阱**不再只靠文档**：codegen 期做一次静态检查（`asm_proc.hpp` 的 `asmCheckAccumAlias`），
命中报 **`VB3041 SemAsmAccumAliasClobber`**。判据是"**别名 + 块内顺序可达**"这个交集：

1. 块里有一条带**隐含累加器**的指令（`cmpxchg`/`xadd`→RAX、`mul`/`imul`/`div`/`idiv`→RAX、`mulx`→RDX）；
2. 在该指令**之前**，同一个寄存器族先有一条**整宽写 W**（`mov rax, r10` —— 建立有意义的 64 位值，
   典型是"把指针/基址放进 RAX"），之后又有一条**短宽写 L**（`mov eax, 0` —— 只写低位，
   32 位写还会把高 32 位清零）；
3. 即 `L 在 W 之后` → 高位回不来了，而这条指令正把该寄存器当累加器用 → 报。

x86 判据同构（族换成 32 位拼写：`mov eax, ebx` 是整宽、`mov ax, 0` 是短宽）。两架构在
...
| `[X]` 的 X 既不是寄存器，也不是可见的 VB 变量/参数 | **3039** |
| x64 单个片段引用 > 4 个 VB 变量 | **3040** |
...
| `asm_mixed_neg_unresolved_ref` | 3039 |
| `asm_mixed_neg_ref_limit` | 3040（x64 专属） |
