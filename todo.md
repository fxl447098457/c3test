...
3. [P1] 绘图语句与画布属性（PictureBox 画图类程序的死穴）
   ...
   - **For Each 同系列（群友补充，详见 ai/胎神建议/005）**：接口 = 标准 IEnumVARIANT（IID {00020404-0000-0000-C000-000000000046}，群友 tlb 里定义名 EnumVariant）。C3 消费侧已实现（P22-11：vb6_ForEach_Init 调 DISPID -4 取 _NewEnum → QI IID_IEnumVARIANT → Next，见 vb6com.c 1469-1601）；**缺口在生产侧**：`Implements IEnumVARIANT/EnumVariant` 通道、类模块 `_NewEnum` 成员（dispid -4）生成、`--dll` typelib 输出 `_NewEnum` 成员使 VB6 可 For Each C3 集合类。顺带清理 cgen_stmt.cpp:2406 过时注释
